...
const toastVariants = cva(
  ' data-[swipe=move]:transition-none group relative pointer-events-auto flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full data-[state=closed]:slide-out-to-right-full',
  {
    variants: {
      variant: {
        default: 'bg-background border dark:bg-gray-100 dark:text-gray-900',
        success:
          'group success bg-background border-green-400 bg-green-100 text-green-700',
        error: 'group error border-red-400 bg-red-100 text-red-700',
        warning:
          'group warning border-yellow-400 bg-yellow-100 text-yellow-400',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 hover:text-gray-300 group-[.success]:text-green-600 group-[.warning]:text-yellow-400  transition-opacity group-[.success]:hover:text-green-400 group-[.warning]:hover:text-yellow-400 focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.error]:text-red-600 group-[.error]:hover:text-red-400 group-[.error]:focus:ring-red-600 group-[.error]:focus:ring-offset-red-400',
      className,
    )}
    toast-close=""
    {...props}
